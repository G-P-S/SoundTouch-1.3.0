...
/*
// Processes the sound
static void processOld(SoundTouch *pSoundTouch, WavInFile *inFile, WavOutFile *outFile)
{
    int nSamples;
    int nChannels;
    int buffSizeSamples;
...
static void process(SoundTouch *pSoundTouch)
{
    int nSamples;
    int nChannels;
    int buffSizeSamples;
    SAMPLETYPE sampleBuffer[BUFF_SIZE*10];
	...
	//	num = 512;
      //  nSamples = num / nChannels;
		nSamples =1;
		for(int i=0; i<buffSizeSamples; i++)
		{
			sampleBuffer[i*2] = i;
			sampleBuffer[i*2+1] = i;
		}


        // Feed the samples into SoundTouch processor
        pSoundTouch->putSamples(sampleBuffer, buffSizeSamples);
        ...
        do 
        {
            nSamples = pSoundTouch->receiveSamples(sampleBuffer, buffSizeSamples*10);
    //        outFile->write(sampleBuffer, nSamples * nChannels);
        } while (nSamples != 0);
    }
...
//    printf(_helloText, SoundTouch::getVersionString());
    ...
    try 
    {
		...
		SoundTouch.setTempoChange(-4.0);
		SoundTouch.setPitchSemiTones(0);
		SoundTouch.setRateChange(0);

		SoundTouch.setSetting(SETTING_USE_QUICKSEEK, false/*params->quick*/);
		SoundTouch.setSetting(SETTING_USE_AA_FILTER, true /*!params->noAntiAlias*/);
